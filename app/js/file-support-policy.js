...
(function (root, factory) {
  ...
  const EXTENSIONS = {
    ...
    pptx: {
      ...
      notes: 'Renders slides from the raw OOXML with theme/layout/master-inherited backgrounds and colors (including real multi-stop gradients), real per-run font size/bold/italic/underline/color, shape position/rotation/fill/border, text-less autoshapes (icons/dividers/card backgrounds), grouped shapes with real nested-transform positioning, connectors/arrows, embedded charts (bar/line/pie/doughnut/area as real charts; other types as a data table), SmartArt diagrams, RTL, bullets, tables, image cropping, and shrink-to-fit text (both PowerPoint\'s own stored autofit and a live fallback pass). Does NOT reproduce actual PowerPoint fonts (browser substitutes), radial/picture backgrounds beyond a linear-gradient approximation, shadows/3D effects, exact bent-connector routing, or animations/transitions — partial fidelity by design, not a bug.',
    },
    ppsx: {
