...
check('excel/pptx capability tables do NOT falsely claim zoom (neither render function wires it up today)', () => {
  ...
  const pptxBody = extractFunctionBody(viewerSrc, /async function renderPptx\(\)\s*\{/);
  ...
  assert(!/addZoomControls/.test(pptxBody), 'renderPptx() unexpectedly has zoom now — update VIEWER_CAPABILITIES_BY_ENGINE[\'pptx-text-extract\'] to add zoom:true');
